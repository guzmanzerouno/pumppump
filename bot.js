...
async function analyzeTransaction(signature, forceCheck = false) {
  ...
  bot.on("callback_query", async (query) => {
    ...
    if (data.startsWith("refresh_")) {
      ...
      try {
        if (buyReferenceMap[chatId]?.[mint]?.messageId === messageId) {
          // Es una confirmación de compra
          await refreshBuyConfirmation(chatId, messageId, mint);
        } else {
          // Es un mensaje de análisis de token
          ...
          try {
            ...
            console.error("❌ Error actualizando mensaje:", editError.message);
            await bot.answerCallbackQuery(query.id, { text: "Error al actualizar." });
          }
        }
      } catch (err) {
        console.error("❌ Error en botón refresh:", err.message);
        await bot.answerCallbackQuery(query.id, { text: "Error general al actualizar." });
      }
  
      return;
    }
  
    // Aquí puedes manejar otros tipos de callbacks si es necesario
  });
  ...
  *SOL/${tokenSymbol}* (${escapeMarkdown(swapDetails.dexPlatform || "Unknown DEX")})
  ...
  🔗 *Wallet:* \`${swapDetails.walletAddress}\``;
    ...
    console.log(`✅ Swap confirmed and reference saved for ${tokenSymbol}`);
     ...
     async function refreshBuyConfirmation(chatId, messageId, tokenMint) {
            const ref = buyReferenceMap[chatId]?.[tokenMint];
            if (!ref) {
              console.log("❌ No buy reference found for:", tokenMint);
              return;
            }
          
            const { gotTokens, tokenPriceBuy, spentTotal, messageText } = ref;
